...
describe("NFT2771 Contract", async () => {
	let accounts, nftContract, forwarder, testnetDai
	...
	before("before test", async () => {
		...
		nftContract = await nftContractFactory.deploy(forwarder.address)
		await nftContract.deployed()
	})

	describe("personal sign", function () {
		it("executes call successfully", async function () {
			const req = await nftContract.populateTransaction.createEternalNFT()
